<script lang="ts" module>
  ...
  export const statusStyles = STATUS_STYLES
</script>

<script lang="ts">
  import { cn } from '$lib/utils'

  type Props = { status: string; class?: string }
  let { status, class: className }: Props = $props()
</script>

<span
  class={cn(
    'inline-flex items-center rounded-full border px-2 py-0.5 text-[11px] font-medium capitalize tracking-wide',
